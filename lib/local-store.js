...
async function loadPublicConfig(userDataPath) {
  ...
  return {
    ...
    useRpcLimiter: Boolean(raw?.useRpcLimiter),
    profiles,
  };
}
...
async function savePublicConfig(userDataPath, payload) {
  ...
  const config = { rpcUrl, useRpcLimiter: Boolean(payload?.useRpcLimiter), profiles };
  await fs.mkdir(userDataPath, { recursive: true });
  await fs.writeFile(path.join(userDataPath, 'config.json'), JSON.stringify(config, null, 2), { encoding: 'utf8', mode: 0o600 });
  return config;
}

module.exports = { loadPublicConfig, loadRecipients, savePublicConfig, saveRecipient };
