...
function decodeSecretText(rawSecret) {
  ...
  if (!text) throw new Error('The selected hot-wallet key file is empty.');
  ...
  try {
    ...
    throw new Error(`The selected file does not contain a valid Solana keypair: ${error?.message || String(error)}`);
  }
}
...
async function importHotWallet(userDataPath, safeStorage, rawSecret, expectedPublicKey = '') {
  if (process.platform !== 'win32') {
    throw new Error('GM hot-wallet import is only enabled in the native Windows app so DPAPI protection is guaranteed.');
  }
  if (!safeStorage.isEncryptionAvailable()) {
    throw new Error('Windows protected storage is not available. The hot-wallet key was not imported.');
  }
  const keypair = decodeSecretText(rawSecret);
  if (expectedPublicKey && keypair.publicKey.toBase58() !== expectedPublicKey) {
    throw new Error('Selected keypair does not match the configured GM hot-wallet address. Nothing was imported.');
  }
...
module.exports = { decodeSecretText, getHotWalletStatus, importHotWallet, loadHotWallet };
