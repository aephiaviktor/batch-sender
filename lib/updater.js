...
function buildWindowsInstallerScript({ sourcePath, destinationPath, stagingPath, logPath, parentPid, expectedVersion }) {
  ...
  return [
    ...
    "  $electron = Join-Path $destination 'node_modules\\electron\\dist\\electron.exe'",
    "  if (-not (Test-Path -LiteralPath $electron -PathType Leaf)) { throw 'Electron binary is missing after update.' }",
    "  Write-UpdaterLog \"Installed version $installedVersion; restarting Electron directly.\"",
    "  Start-Process -FilePath $electron -ArgumentList @(\"`\"$destination`\"\") -WorkingDirectory $destination",
